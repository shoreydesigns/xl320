...
void setup() {
  ...
  Dxl.writeWord(1,32,2047); //RW Moving Speed(Velocity) 
  Dxl.writeWord(2,32,2047); //RW Moving Speed(Velocity) 
  
  Dxl.writeByte (254, 24, 1); // Enable Torque for all DYNAMIXELs listening and get started!
   Dxl.writeWord (254, 15, 1023);    //  Max torque
  delay(100);
  
  Dxl.writeWord (2, 30, 512); // all servos desired position 512
  delay(3000);
  Dxl.writeWord (1, 30, 512); // all servos desired position 512
  delay(3000);
  
 
  //Dxl.writeWord (254, 15, 200);    //  Max torque
   
   // !?! 1 and 2 !?!?!
  Dxl.writeByte (2, 24, 0);    // servo 1  disable torque 
  Dxl.writeByte (1, 24, 0);    // servo 2  disable torque 
  delay(100); 
...
void setTorque(unsigned char servo,  int V) {
   V = Limit(ABS(V), 0, 500); // limit torque to 900 (?)
